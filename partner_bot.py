...
def listing_description_length_rules(raw_text):
    ...
    if (
        ...
        return (
            ...
            "сделай компактным, ориентируясь примерно на 30-35 слов. Это мягкий ориентир только для вводного описания, "
            "а не ко всему объявлению. Удали повторы и рекламную воду, но сохрани все уникальные важные факты "
            "в подходящих разделах и финальных условиях. Естественность текста и сохранность фактов важнее механического подсчёта слов."
        )
    return (
        "- Исходник короткий или обычный по объёму: ориентир для вводного описания — примерно 30-35 слов, но качество "
        "и естественность важнее точного числа. Короткий текст можно умеренно расширить и плавно связать его факты: "
        "например, описание на 25 слов можно довести примерно до 30, если оно станет лучше. Не добавляй выдуманные "
        "характеристики, неподтверждённые преимущества или пустые фразы только ради объёма. Плохой, обрывочный или "
        "иностранный текст можно полностью переформулировать, сохраняя реальный масштаб и содержание исходника. "
...
- Объём только вводного описания выбирай по количеству фактов. Обычно это 1-4 связных предложения и примерно 30-35 слов. Это не жёсткие рамки: отклонение на несколько слов допустимо, если оно делает русский текст естественнее, красивее или помогает не потерять важный факт. Но не превращай мягкий ориентир в описание на 50-60 слов, если те же факты можно выразить яснее. Второстепенные характеристики и условия — подвал, парковку, животных, интернет, дату заезда — при необходимости вынеси в короткие финальные фразы после основных разделов. Разделы с локацией, финансами и условиями в эти слова не входят.
...
async def setup_bot_commands(app):
    ...
    try:
        await app.bot.set_my_commands([
            BotCommand("start", "Начать публикацию объявления"),
            BotCommand("partner", "Новое объявление"),
...
async def stop_background_tasks(app):
    ...
    global CHANNEL_FULL_SYNC_TASK, HEALTH_WEB_RUNNER
...
async def my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Короткая личная статистика партнёра. Только его собственные данные."""
    user = update.effective_user
    if not user:
        return
    if is_admin(user.id):
    ...
    counters = stats_counters(stats_user_items(context, user.id))
    ...
    employee_code = employee_owner_code or (
        linked_employee_code if linked_employee_code in EMPLOYEES else None
    )

    lines = ["<b>Моя статистика</b>"]
    if employee_code:
        lines.append(f"Контакт в объявлениях: {html.escape(employee_display_name(employee_code))}")
    lines.append("")
    lines.append(f"Объявлений: {counters['total']}")
    lines.append(f"На проверке и дальше: {counters['submitted']}")
    ...
    if employee_owner_code:
        # Сотрудник Binio дополнительно видит итог по своей ссылке.
        employee_items = [
            item
            for item in stats_employee_items(context, employee_owner_code)
            if str(item.get("partner_id")) != str(user.id)
        ]
        employee_counters = stats_counters(employee_items)
        linked_users = stats_employee_linked_user_ids(context, employee_owner_code, user.id)
        # Пустой блок из одних нулей только мешает читать сводку.
        if linked_users or employee_counters["total"]:
            lines.append("")
            lines.append("<b>По вашей ссылке</b>")
            lines.append(f"Партнёров: {len(linked_users)}")
            lines.append(f"Объявлений: {employee_counters['total']}")
            lines.append(f"Опубликовано: {employee_counters['published']}")
            lines.append(f"Сейчас в канале: {employee_counters['active']}")
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for employee_key in EMPLOYEE_CHOICE_KEYS:
        ...
        if not row["listings"] and not row["partners"]:
            continue
        employee_lines.append(
            f"— {html.escape(employee_display_name(employee_key))}: "
            f"партнёров {len(row['partners'])}, "
            f"объявлений {len(row['listings'])}, "
    ...
    lines = [
        ...
        "",
        f"Ждут решения: {pending_submitted}",
        f"Черновики: {pending_drafts}",
        "",
        f"Объявлений всего: {counters['total']}",
...
async def admin_stats_full(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    employee_lines = [
        f"— {html.escape(employee_display_name(employee_key))}: "
        f"партнёры {len(employee_rows[employee_key]['partners'])}, "
        f"объявления {len(employee_rows[employee_key]['listings'])}, "
        f"на проверке {employee_rows[employee_key]['submitted']}, "
        f"публикации {employee_rows[employee_key]['published']}"
        for employee_key in EMPLOYEE_CHOICE_KEYS
    ]
    ...
    text = (
        ...
        f"— опубликовано: {counters['published']}\n"
        f"— отклонено: {counters['rejected']}\n"
        f"— конверсия проверка → публикация: "
        f"{stats_percent(counters['published'], counters['submitted'])}\n\n"
