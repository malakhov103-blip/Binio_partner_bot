...
# Gemini 2.5 Flash без явной настройки использует динамическое размышление и
# может тратить на простую редактуру неоправданно много времени. Оставляем
# консервативный бюджет: он сохраняет возможность рассуждать над переводом и
# сложным исходником, но не позволяет внутреннему анализу разрастаться.
GEMINI_THINKING_BUDGET = int_env("GEMINI_THINKING_BUDGET", 1024, minimum=0, maximum=4096)
_gemini_generation_kwargs = {
    "temperature": 0.7,
    "top_p": 0.9,
...
_thinking_fields = getattr(genai.types.ThinkingConfig, "model_fields", None)
if _thinking_fields is None:
    _thinking_fields = getattr(genai.types.ThinkingConfig, "__fields__", {})
if "thinking_budget" in _thinking_fields:
    _gemini_generation_kwargs["thinking_config"] = genai.types.ThinkingConfig(
        thinking_budget=GEMINI_THINKING_BUDGET,
    )
else:
    # Старые сборки Railway могли оставить google-genai 1.2.0. В этой версии
    # thinking_budget ещё отсутствует: ускорение выключается, но бот обязан
    # запуститься и продолжить работать с прежним качеством Gemini.
    logger.warning(
        "Установленная версия google-genai не поддерживает thinking_budget; "
        "Gemini работает в прежнем динамическом режиме"
    )
GEMINI_GENERATION_CONFIG = genai.types.GenerateContentConfig(**_gemini_generation_kwargs)
