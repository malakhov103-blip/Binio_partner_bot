...
def build_gemini_generation_config(thinking_config_type=None):
    """Создаёт ускоренную конфигурацию, не ломая запуск со старым SDK."""
    config_kwargs = {
        "temperature": 0.7,
        "top_p": 0.9,
    ...
    thinking_type = thinking_config_type or genai.types.ThinkingConfig
    supported_fields = getattr(thinking_type, "model_fields", {})
    if "thinking_budget" in supported_fields:
        # Gemini 2.5 Flash размышляет по умолчанию и без ограничения иногда
        # тратит на редакторскую задачу лишние секунды. Анализ не отключаем.
        # 512 токенов сохраняют проверку языка, фактов и структуры, но не
        # заставляют короткое объявление ждать избыточный бюджет в 1024 токена.
        ...
        logger.warning(
            "Установленная версия google-genai не поддерживает thinking_budget; "
            "бот продолжит работу без ускорения Gemini"
        )
    return genai.types.GenerateContentConfig(**config_kwargs)


GEMINI_GENERATION_CONFIG = build_gemini_generation_config()
