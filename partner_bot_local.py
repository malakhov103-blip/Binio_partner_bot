...
PUBLIC_GEMINI_DAILY_LIMIT = max(0, int(os.getenv("PUBLIC_GEMINI_DAILY_LIMIT", "15")))
...
CHANNEL_AUTO_SYNC_INTERVAL_SECONDS = max(900, int(os.getenv("CHANNEL_AUTO_SYNC_INTERVAL_SECONDS", str(6 * 60 * 60))))
CHANNEL_AUTO_SYNC_MAX_ITEMS = max(1, min(20, int(os.getenv("CHANNEL_AUTO_SYNC_MAX_ITEMS", "8"))))
CHANNEL_CHECK_VERSION = "telegram-preview-v2"
...
GEMINI_CONCURRENT_LIMIT = max(1, min(16, int(os.getenv("GEMINI_CONCURRENT_LIMIT", "8"))))
GEMINI_SEMAPHORE = asyncio.Semaphore(GEMINI_CONCURRENT_LIMIT)
MAX_CONCURRENT_UPDATES = max(4, min(32, int(os.getenv("MAX_CONCURRENT_UPDATES", "16"))))
...
GEMINI_TIMEOUT_SECONDS = max(20, min(25, int(os.getenv("GEMINI_TIMEOUT_SECONDS", "25"))))
GEMINI_MAX_ATTEMPTS = max(1, min(2, int(os.getenv("GEMINI_MAX_ATTEMPTS", "2"))))
GEMINI_SLOW_NOTICE_SECONDS = max(5, min(10, int(os.getenv("GEMINI_SLOW_NOTICE_SECONDS", "8"))))
STRIPE_API_TIMEOUT_SECONDS = max(5, min(30, int(os.getenv("STRIPE_API_TIMEOUT_SECONDS", "15"))))
...
def consume_public_gemini_request(context, user_id):
    """Ограничивает только бесплатную генерацию черновиков собственника, не партнёров."""
    if is_admin(user_id) or PUBLIC_GEMINI_DAILY_LIMIT <= 0:
        return False
    key = f"public_gemini_usage_{user_id}"
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    usage = context.application.bot_data.get(key)
    if not isinstance(usage, dict) or usage.get("date") != today:
        usage = {"date": today, "count": 0}
    if int(usage.get("count", 0)) >= PUBLIC_GEMINI_DAILY_LIMIT:
        return False
...
async def _paid_delivery_job(app, listing_id):
    try:
        ...
        if not pending or not pending.get("paid") or pending.get("submitted_to_admin"):
            return
    ...
    while True:
        try:
            for listing_id, pending in list_unique_pending_items(context_from_app(app)).items():
                if pending.get("source") == "public" and pending.get("paid") and not pending.get("submitted_to_admin"):
...
def start_stripe_delivery_recovery(app):
    ...
    if STRIPE_RECOVERY_TASK is None or STRIPE_RECOVERY_TASK.done():
        creator = getattr(app, "create_task", None)
        coroutine = stripe_delivery_recovery_loop(app)
        STRIPE_RECOVERY_TASK = (
            creator(coroutine, name="stripe-delivery-recovery")
            if creator is not None
            else asyncio.create_task(coroutine, name="stripe-delivery-recovery")
        )
...
def remember_stripe_event(app, event_id):
    ...
    if len(events) > 200:
        oldest = sorted(events.items(), key=lambda item: item[1])[:-200]
...
async def handle_stripe_checkout_completed(app, session, background=False):
    ...
    if not pending or pending.get("source") != "public":
        ...
        try:
            await app.bot.send_message(
                ...
                text=f"⚠️ Stripe подтвердил оплату, но заявка не найдена: listing_id={listing_id}, user_id={user_id}",
            )
        except Exception as notify_error:
            logger.error(f"Stripe webhook: не удалось предупредить администратора: {notify_error}")
        try:
            await app.bot.send_message(
                chat_id=user_id,
                text=(
                    "✅ Оплата подтверждена, но бот не нашёл сохранённое объявление.\n\n"
                    "Администратор уведомлён. Повторно оплачивать не нужно; при обращении в поддержку укажите время оплаты."
                ),
            )
        except Exception as user_notify_error:
            logger.error(f"Stripe webhook: не удалось уведомить оплатившего пользователя: {user_notify_error}")
        return False
    if pending.get("partner_id") != user_id:
        ...
        try:
            await app.bot.send_message(
                chat_id=ADMIN_CHAT_ID,
                text=f"⚠️ Stripe user_id не совпал с владельцем объявления: listing_id={listing_id}, user_id={user_id}",
            )
        except Exception as notify_error:
            logger.error(f"Stripe webhook: не удалось предупредить о user_id mismatch: {notify_error}")
        return False
    ...
    if amount_total != PUBLIC_PAYMENT_AMOUNT or currency != PUBLIC_PAYMENT_CURRENCY:
        ...
        try:
            ...
            logger.error(f"Stripe webhook: не удалось уведомить пользователя о проверке суммы: {user_notify_error}")
        return False

    # Оплаченный Stripe Checkout нельзя отклонять из-за локального TTL или
    # месячного лимита: деньги уже списаны. Эти проверки выполняются до оплаты.
    # После подтверждения обязаны либо отправить объявление, либо явно уведомить
    # пользователя и администратора о проблеме.
    ...
    processed = await submit_paid_public_listing(
        context_from_app(app),
        listing_id,
        PaidUser(user_id),
        reply_target,
...
def published_status_label(status, channel_missing=False):
    if channel_missing:
        return "⚫ Нет в канале"
    if status == "rented":
        return "🔴 Сдано"
    ...
    try:
        async with session.get(preview_url, allow_redirects=True) as response:
            ...
            if response.status >= 400:
                return None
            # Telegram may return HTTP 200 with the generic channel page when
            # the post does not exist. A real preview contains the exact
            # data-post marker for this channel and message ID.
...
async def verify_published_page(context, listings, page):
    ...
    return await verify_published_items(context, page_items)
...
async def full_channel_sync_job(app, admin_chat_id):
    ...
    try:
        ...
        items = [
            ...
            and not value.get("hidden_from_list")
        ]
        total_missing = 0
        total_unknown = 0
        checked = 0
...
def published_list_keyboard(listings, page=0):
    ...
    for item in page_items:
        ...
        rows.append([
            InlineKeyboardButton(
                f"{published_status_label(item.get('status'), item.get('channel_missing'))} · {headline}",
                callback_data=f"pub_view_{item['listing_id']}",
            )
        ])
    navigation = []
    if page > 0:
        navigation.append(InlineKeyboardButton("⬅️ Назад", callback_data=f"my_listings_page_{page - 1}"))
    if page < total_pages - 1:
        navigation.append(InlineKeyboardButton("Далее ➡️", callback_data=f"my_listings_page_{page + 1}"))
    if navigation:
...
def published_manage_keyboard(item, list_page=0):
    listing_id = item["listing_id"]
    rows = []
    if item.get("channel_missing"):
        rows.append([
            InlineKeyboardButton("🗑️ Убрать из списка", callback_data=f"pub_hide_{listing_id}")
        ])
    ...
    rows.append([InlineKeyboardButton("📋 К списку", callback_data=f"my_listings_page_{list_page}")])
    return InlineKeyboardMarkup(rows)
...
def published_card_text(item):
    ...
    status = published_status_label(item.get("status"), item.get("channel_missing"))
...
async def generate_formatted_listing(raw_text, property_type_key, contact_url, status_message=None):
        ...
        for attempt in range(GEMINI_MAX_ATTEMPTS):
            ...
            try:
                ...
                if attempt < GEMINI_MAX_ATTEMPTS - 1:
                    ...
                    if remaining <= 0:
                        break
                    await asyncio.sleep(min(0.8, remaining))
                    continue
    finally:
    ...
    remaining = GEMINI_TIMEOUT_SECONDS - (time.monotonic() - overall_started)
    if remaining <= 0:
        return prepare_listing_without_gemini_shortening(formatted_listing, contact_url)
    try:
        return await asyncio.wait_for(
            prepare_listing_for_caption(formatted_listing, contact_url),
            timeout=remaining,
        )
    except asyncio.TimeoutError:
...
async def process_listing(update, context, text):
    ...
    try:
        ...
        if existing_pending:
            ...
            employee_code = session_partner_code or context.application.bot_data.get(f"partner_code_{user_id}")
...
def stats_employee_code(item):
    ...
    if item.get("source") == "partner":
        ...
        if contact_url and contact_url != DEFAULT_CONTACT:
            code = employee_key_by_contact(contact_url)
            if code in EMPLOYEES:
                return code
    return None
...
def stats_revenue_czk(item):
    ...
    try:
        if amount is not None:
            return max(0.0, float(amount) / 100.0)
    except (TypeError, ValueError):
        pass
...
def stats_all_items(context):
    """Единый список объявлений без двойного счёта pending + published."""
    items = {}
    for listing_id, pending in list_unique_pending_items(context).items():
        row = dict(pending)
...
def stats_employee_linked_user_ids(context, employee_code, exclude_user_id=None):
    ...
    for key, value in context.application.bot_data.items():
        if not key.startswith(prefix) or value != employee_code:
            continue
...
async def my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    linked_employee_code = context.application.bot_data.get(f"partner_code_{user.id}")
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"— реальных оплат Stripe: {real_paid_total}\n"
...
async def partner_my_listings(
    ...
    text = notice_prefix + (
        f"Мои объявления · всего: {len(listings)}\n"
        ...
        "Проверка наличия постов в канале выполняется автоматически."
    )
    keyboard = published_list_keyboard(listings, page)
    if update.callback_query:
        try:
...
async def partner_published_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("my_listings_check_"):
        ...
        missing, unknown = await verify_published_page(context, listings, page)
        if missing:
            notice = (
                f"Проверка завершена: {missing} объявлений отмечено как отсутствующие в канале.\n"
                "Откройте такую карточку и уберите её из списка. История и статистика сохранятся."
            )
        elif unknown:
        ...
        await partner_my_listings(update, context, page=page, notice=notice, answer_query=False)
        return
    ...
    if data == "my_listings" or data.startswith("my_listings_page_"):
        ...
        if data.startswith("my_listings_page_"):
            try:
                page = int(data.replace("my_listings_page_", "", 1))
            except ValueError:
                page = 0
        await partner_my_listings(update, context, page=page)
        return
    ...
    if data.startswith("pub_hide_"):
        ...
        if not item.get("channel_missing"):
            ...
            return
        list_page = published_page_for_listing(list_partner_published(context, user_id), listing_id)
        item["hidden_from_list"] = True
    ...
    if data.startswith("pub_view_"):
        ...
        try:
            await query.edit_message_text(
                ...
                parse_mode="HTML",
                disable_web_page_preview=True,
                reply_markup=published_manage_keyboard(item, list_page),
            )
        except Exception:
            await query.message.reply_text(
                text=text,
    ...
    if data.startswith("pub_rented_") or data.startswith("pub_active_"):
        ...
        try:
            await query.edit_message_text(
                ...
                parse_mode="HTML",
                disable_web_page_preview=True,
                reply_markup=published_manage_keyboard(updated, list_page),
            )
        except Exception:
            await query.message.reply_text(
                text=text,
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(
        ...
        pattern=r"^(my_listings(?:_page_[0-9]+|_check_[0-9]+)?|pub_view_[A-Za-z0-9_-]+|pub_hide_[A-Za-z0-9_-]+|pub_rented_[A-Za-z0-9_-]+|pub_active_[A-Za-z0-9_-]+|pub_money_(price|deposit|commission)_[A-Za-z0-9_-]+)$"
    ))
