...
def normalize_prague_area_hashtag(text):
    match = re.search(r'\bPra(?:ha|gue)\s*([1-9]|10)\b|\bПрага\s*([1-9]|10)\b', text, flags=re.I)
    if not match:
        return text

    district = match.group(1) or match.group(2)
    prague_tag = f"#Praha{district}"
    known_type_tags = {
        "#1kk", "#1plus1", "#2kk", "#2plus1", "#3kk", "#3plus1", "#4kk", "#4plus1", "#5kk",
        "#pokoj", "#dum", "#pozemek", "#komerce",
    }
    ...
    if any(tag.lower() == prague_tag.lower() for tag in tags):
        return re.sub(r'#praha([1-9]|10)\b', lambda m: f"#Praha{m.group(1)}", text, flags=re.I)

    for tag in tags:
        lower = tag.lower()
        if lower in known_type_tags or lower == "#pronajem":
            continue
        return re.sub(re.escape(tag), prague_tag, text, count=1)

    return text.rstrip() + f" {prague_tag}"
...
async def prepare_listing_for_caption(text, contact_url):
    ...
    """
    prepared = normalize_prague_area_hashtag(normalize_layout_hashtags(normalize_russian_headline(text)))
    for _ in range(2):
        prepared = normalize_prague_area_hashtag(normalize_layout_hashtags(normalize_russian_headline(prepared)))
