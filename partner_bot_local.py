...
def filter_published_listings(listings, filter_key):
    ...
    if filter_key == "all":
        # Основной список не должен снова заполняться вручную архивированными
        # карточками. Историю можно открыть отдельным фильтром «Архив».
        return [item for item in listings if not item.get("hidden_from_list")]
    ...
    if filter_key == "rented":
        return [
            item for item in listings
            if not item.get("hidden_from_list") and item.get("status") == "rented"
        ]
    return [
        item for item in listings
...
def cleanup_bot_memory(bot_data):
    ...
    for key in list(bot_data.keys()):
        ...
        if pending.get("submitted_to_admin"):
            ...
            # Оплаченные, но ещё не отправленные заявки нельзя автоматически
            # удалять: это финансовая история и основание для повторной доставки.
            # После исправления пользователь сможет отправить её кнопкой ещё раз.
            pass
...
def published_list_keyboard(listings, page=0, filter_key="all", counts=None):
    ...
    rows.append([
        InlineKeyboardButton(
            f"{('✅ ' if filter_key == 'all' else '')}Все · {counts['all']}",
...
async def submit_paid_public_listing(
    ...
    try:
        ...
        clear_pending_busy(context, listing_id, pending, "submit_in_progress")
        should_notify = not background_delivery or not pending.get("paid_delivery_notice_sent")
        if should_notify:
            pending["paid_delivery_notice_sent"] = True
            save_pending(context, listing_id, pending)
            try:
...
async def partner_my_listings(
    ...
    text = notice_prefix + (
        f"Мои объявления · {PUBLISHED_LISTING_FILTERS[filter_key]}\n"
        f"В разделе: {len(listings)} · Всего: {len(all_listings)}\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(
        ...
        pattern=r"^(my_listings(?:_page_(?:[0-9]+|(?:all|active|rented|archive)_[0-9]+)|_check_[0-9]+|_filter_(?:all|active|rented|archive)|_new)?|pub_view_[A-Za-z0-9_-]+|pub_hide_[A-Za-z0-9_-]+|pub_archive_[A-Za-z0-9_-]+|pub_unarchive_[A-Za-z0-9_-]+|pub_rented_[A-Za-z0-9_-]+|pub_active_[A-Za-z0-9_-]+|pub_money_(price|deposit|commission)_[A-Za-z0-9_-]+)$"
    ))
