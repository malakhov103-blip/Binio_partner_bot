...
MAIN_CHANNEL_START_PARAM = "channel"
...
def main_channel_bot_url():
    """Neutral entry point: it never grants access to a particular employee."""
    return f"https://t.me/{BOT_USERNAME}?start={MAIN_CHANNEL_START_PARAM}"
...
async def handle_stripe_checkout_completed(app, session):
    ...
    return await submit_paid_public_listing(
    ...
    if event.get("type") == "checkout.session.completed":
        try:
            await asyncio.wait_for(
                handle_stripe_checkout_completed(request.app["telegram_app"], event["data"]["object"]),
                timeout=STRIPE_FULFILLMENT_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
            logger.error(
                "Stripe webhook: Telegram не завершил доставку за %.1fс; Stripe повторит событие",
                STRIPE_FULFILLMENT_TIMEOUT_SECONDS,
            )
            return web.Response(status=500, text="temporary processing timeout")
        except Exception:
            # Ответ 500 заставит Stripe повторить доставку события. Полный traceback
            # нужен в Railway Logs, иначе сбой после оплаты невозможно расследовать.
    ...
    if STRIPE_ENABLED and session_id.startswith("cs_") and len(session_id) <= 255:
        try:
            ...
            processed = await asyncio.wait_for(
                handle_stripe_checkout_completed(request.app["telegram_app"], session),
                timeout=STRIPE_FULFILLMENT_TIMEOUT_SECONDS,
            )
            if processed:
                page_heading = "Оплата подтверждена"
                reconciliation_message = "Оплата подтверждена. Объявление отправлено на проверку."
