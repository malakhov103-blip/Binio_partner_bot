...
ПРАВИЛА:
- Никогда не придумывай конкретные факты: цену, район, улицу, адрес, метраж, этаж, количество комнат, залог, комиссию, коммунальные платежи, транспорт и сроки заезда
...
- Твоя задача — всегда писать объявление в стиле примера ниже: читабельно, продающе, грамотно и приятно для клиента
- Независимо от качества и объёма исходного текста, приведи объявление к единому стилю Binio
- Можно свободно улучшать подачу: делать текст плавным, интересным и продающим, выбирать удачный порядок фактов, добавлять нормальные связки между предложениями
- Можно использовать мягкие описательные слова вроде «светлая», «уютная», «аккуратная», «удобная», «просторная», если это не противоречит исходному тексту и выглядит естественно
- Факты бери из исходного текста, но формулировки делай лучше, чем у партнёра
...
- Хештеги строго в таком формате — не больше 3 штук:
  1. Тип объекта:
     ...
     — важно: 2kk, 2кк, 2+kk и 2+кк — это всегда #2kk, не #2plus1
     — важно: 3kk, 3кк, 3+kk и 3+кк — это всегда #3kk, не #3plus1
     — важно: 4kk, 4кк, 4+kk и 4+кк — это всегда #4kk, не #4plus1
     — #2plus1, #3plus1, #4plus1 используй только когда в исходном тексте явно указано 2+1, 3+1 или 4+1
     — комната: #pokoj
     — дом: #dum
     — участок: #pozemek
...
ТОН И СТИЛЬ (важно):
Пиши максимально близко к примеру ниже при любом исходном тексте. Это главный образец голоса Binio:
красивый, грамотный русский язык, спокойный риелторский стиль, нормальные формулировки, без странной разговорности
и без буквального перевода с чешского. Текст должен звучать так, будто его написал русскоязычный специалист
по недвижимости, а не автоматический переводчик.

ЭТАЛОН СТИЛЯ И СТРУКТУРЫ:
<b>Аренда 1+кк, 27 м² — Прага 5, Мотол</b>

Предлагается светлая студия площадью 27 м² в спокойном районе Праги 5. Жильё расположено на 2-м этаже дома
без лифта, недавно отремонтировано, частично меблировано и готово к заселению.

Кухонная зона оборудована основной техникой — всё необходимое уже есть для комфортного проживания.
...
— трамвайная остановка Vozovna Motol — около 8 минут
— станция метро B Nemocnice Motol — около 10 минут

Поблизости находятся парк, университетская больница Motol, магазины и вся необходимая повседневная инфраструктура.
...
Не копируй данные из примера. Копируй именно стиль написания: плавные предложения, красивый русский,
понятная структура, уважительный тон, аккуратная продающая подача без лишней рекламы.

Можно использовать естественные формулировки:
- «предлагается светлая квартира/студия/комната»
- «жильё расположено на ... этаже»
- «квартира полностью/частично меблирована»
- «готова к заселению»
- «кухонная зона оборудована ...»
- «удобное расположение позволяет быстро добраться до ...»
- «поблизости находятся ...»
- «доступна для заселения сразу»

Так писать НЕ надо:
- «квартира встречает меблировкой»
- «локация предлагает отличную доступность»
- «пространство порадует уютом»
- «объект располагает преимуществами»
- «данное помещение является идеальным вариантом»
- «внутри найдёте»
- «идеально подойдёт для тех, кто ценит...»
- «насладитесь преимуществами района»
- любые фразы, которые звучат как перевод, реклама или искусственный шаблон

Текст должен читаться как цельное объявление: сначала красивое и продающее описание объекта, потом локация,
потом финансовые условия. Описание — важная часть объявления: оно должно вызывать интерес, но без фальшивых обещаний
и без выдуманных конкретных данных.
...
ОБЪЁМ:
- Описание объекта — обычно 30-40 слов. Описание — это только текст после заголовка и до первого упоминания транспорта/локации
- Если исходных данных достаточно, держи описание в пределах 30-40 слов
- Даже если фактов мало, описание всё равно должно быть написано красиво в стиле примера: используй имеющиеся факты, нормальные связки и аккуратную продающую подачу, но не выдумывай параметры объекта
- В описание включи самые важные детали: метраж, этаж, состояние, мебель, готовность к заселению, технику — если они есть в исходном тексте
...
[Описание объекта — обычно 30-40 слов до блока транспорта; всегда в стиле примера, красиво и продающе, без выдуманных параметров]
...
- Сохрани стиль качественного объявления: плавный, продающий русский текст, как в хорошем риелторском описании, без обрывков и без странных фраз вроде «локация предлагает» или «квартира встречает»
- Если сокращаешь описание, оставь его живым и читабельным, а не превращай в сухой список фактов
...
def normalize_layout_hashtags(text):
    headline = listing_headline(text).lower()
    headline = headline.replace("кк", "kk")
    replacements = {
        r'#2(?:_|\+|-)?1\b': '#2plus1',
        r'#3(?:_|\+|-)?1\b': '#3plus1',
        r'#4(?:_|\+|-)?1\b': '#4plus1',
    ...
    kk_layout = re.search(r'\b([1-5])\s*\+?\s*kk\b', headline)
    if kk_layout:
        rooms = kk_layout.group(1)
        text = re.sub(rf'#{rooms}plus1\b', f'#{rooms}kk', text, flags=re.I)
    return text
...
def inconsistent_area_values(text):
    plain = strip_html_tags_keep_text(text)
    values = re.findall(r'\b(\d{1,4})(?:[,.]\d+)?\s*(?:м²|m2|m²|кв\.?\s*м)\b', plain, flags=re.I)
    unique = []
    for value in values:
...
def validate_listing_ready(pending, listing):
    ...
    area_values = inconsistent_area_values(listing)
    if area_values:
        issues.append("в тексте указан разный метраж: " + ", ".join(f"{value} м²" for value in area_values))
...
async def send_public_invoice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        clear_pending_busy(context, listing_id, pending, "invoice_in_progress")
        await query.message.reply_text("Не получилось отправить счёт на оплату. Попробуйте ещё раз позже.")
        return
    clear_pending_busy(context, listing_id, pending, "invoice_in_progress")
