...
def published_manage_keyboard(item):
    ...
    if item.get("status") == "rented":
        rows.append([InlineKeyboardButton("Вернуть в активные", callback_data=f"pub_active_{listing_id}")])
    else:
...
def listing_fix_keyboard(listing_id):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Изменить текст", callback_data=f"partner_edit_{listing_id}")],
    ])


def admin_fix_keyboard(listing_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Исправить", callback_data=f"edit_more_{listing_id}")],
...
async def prepare_listing_for_caption(text, contact_url):
    ...
    """
    prepared = normalize_listing_hashtags(normalize_russian_headline(text))
    for _ in range(2):
...
async def payment_terms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Оплата списывается только после подтверждения в Telegram. После успешной оплаты объявление отправляется администратору на проверку. "
        "Публикация в канале происходит после одобрения.\n\n"
        ...
        "Telegram не хранит данные банковской карты. Оплату проводит подключённый платёжный провайдер",
        parse_mode="HTML",
        disable_web_page_preview=True,
    )
...
async def admin_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Исправить ещё", callback_data=f"edit_more_{listing_id}")]
    ])
