...
async def stop_stripe_webhook_server(app):
    global STRIPE_WEB_RUNNER, STRIPE_RECOVERY_TASK
...
async def verify_published_page(context, listings, page):
    """Проверяет только текущую страницу, чтобы не замедлять большой список."""
    start = page * PUBLISHED_LISTINGS_PAGE_SIZE
    page_items = listings[start:start + PUBLISHED_LISTINGS_PAGE_SIZE]
    if not page_items or aiohttp is None:
        return 0, len(page_items)
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            results = await asyncio.gather(
                *(check_public_channel_post(session, item) for item in page_items),
    ...
    unknown = 0
    checked_at = now_iso()
    for item, result in zip(page_items, results):
        if result is False:
...
def published_list_keyboard(listings, page=0):
    ...
    rows.append([
        InlineKeyboardButton("🔎 Проверить страницу", callback_data=f"my_listings_check_{page}")
    ])
    navigation = []
    if page > 0:
