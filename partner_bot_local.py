...
GEMINI_MAX_ATTEMPTS = int_env("GEMINI_MAX_ATTEMPTS", 2, minimum=1, maximum=2)
...
async def stripe_webhook(request):
    ...
    if event.get("type") in ("charge.refunded", "charge.dispute.created", "charge.dispute.closed"):
        ...
        if listing_id and record:
            ...
            logger.warning(
                "Stripe status event не сопоставлен с payment record: type=%s payment_intent=%s",
                event.get("type"),
                payment_intent,
            )
...
def cleanup_bot_memory(bot_data):
    ...
    for key in list(bot_data.keys()):
        ...
        if pending.get("paid"):
            ...
            # опубликованную/историческую запись. TTL не может удалить деньги.
            pass
...
async def submit_paid_public_listing(
    ...
    # Для собственника обновляем контакт в последний возможный момент. Если
    # username изменился, публикация не должна содержать устаревшую ссылку;
    # фон Stripe использует стабильный tg://user?id=... контакт.
...
async def admin_retry_publish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pending:
        ...
        return
    if pending.get("publish_state") != "unknown":
