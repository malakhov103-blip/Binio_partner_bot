...
async def my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if counters["submitted"]:
        lines.append("")
        lines.append(
            f"Доходит до канала: {stats_percent(counters['published'], counters['submitted'])} "
            "от отправленного на проверку"
        )
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for employee_key in ranked:
        ...
        own = row["own_listings"]
        partner_listings = len(row["listings"]) - own
