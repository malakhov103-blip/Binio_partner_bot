...
def validate_config():
    ...
    if missing:
        raise RuntimeError(
            ...
            ". Проверьте вкладку Variables именно у этого сервиса. Значения должны быть заполнены, без кавычек и пробелов."
        )
...
async def generate_formatted_listing(raw_text, property_type_key, contact_url, status_message=None):
    ...
    for attempt in range(3):
        ...
        try:
            ...
            last_error = TimeoutError("Gemini не ответил за 25 секунд")
