...
GEMINI_SHORTEN_TIMEOUT_SECONDS = max(5, min(12, int(os.getenv("GEMINI_SHORTEN_TIMEOUT_SECONDS", "12"))))
...
def listing_description_length_rules(raw_text):
    """Ограничивает только вводный абзац и только у действительно огромного исходника."""
    ...
    if (
        ...
        return (
            ...
            "сделай компактным, примерно 30-45 слов. Это ограничение относится только к вводному описанию, "
    ...
    return (
        "- Исходник короткий или обычный по объёму: не сокращай вводное описание до 30-45 слов автоматически. "
        "Выбери естественную длину по количеству полезных фактов."
    )
...
- Не делай краткую сводку. Итог не обязан быть короче исходника: короткий исходник можно аккуратно раскрыть, а подробный — упорядочить и немного сократить без потери содержания.
- Объём описания выбирай по количеству фактов. Обычно это 2-5 связных предложений: без воды и повторов.
...
Заголовок: жирный через <b>, начинается с типа объекта согласно выбранным правилам. Разделы тоже жирные. Без эмодзи и звёздочек.
...
def ensure_contact_line(text, contact_url):
    ...
    return insert_contact_before_hashtags(text, contact_line)
...
async def process_listing(update, context, text):
    ...
    try:
        ...
        if source == "public" and not consume_public_gemini_request(context, user_id):
            set_state(context, user_id, "waiting_text")
            await update_processing_status(
                processing_message,
...
async def partner_regenerate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pending.get("source") == "public" and not consume_public_gemini_request(context, user_id):
        await query.answer(
...
async def admin_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending['editable_listing'] = prepare_listing_for_editing(
        message.text,
        pending.get('contact_url', DEFAULT_CONTACT),
    )
    save_pending(context, listing_id, pending)
