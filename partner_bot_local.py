...
# Используем ту же качественную модель и тот же полный промпт. Две короткие
# попытки сохраняют надёжность при редком сетевом сбое, но ограничивают общее
# ожидание примерно 31 секундой вместо прежних ~68 секунд.
GEMINI_TIMEOUT_SECONDS = max(8, min(30, int(os.getenv("GEMINI_TIMEOUT_SECONDS", "15"))))
...
async def stripe_success_page(request):
    ...
    reconciliation_message = "Бот получил сигнал об оплате. Вернитесь в Telegram."
    if STRIPE_ENABLED and session_id.startswith("cs_") and len(session_id) <= 255:
        try:
            ...
            processed = await handle_stripe_checkout_completed(request.app["telegram_app"], session)
            if processed:
                reconciliation_message = "Оплата подтверждена. Объявление отправлено на проверку."
            else:
                reconciliation_message = "Оплата подтверждается. Проверьте сообщения бота в Telegram."
        except Exception:
